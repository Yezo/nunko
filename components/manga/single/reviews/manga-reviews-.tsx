...
import { IMangaReview, IMangaReviews } from "@/types/manga/type-manga-reviews"

type MangaReviewContainerProps = {
  reviews: IMangaReview[]
  params: { id: string }
}
export const MangaReviewContainer = ({ reviews, params }: MangaReviewContainerProps) => {
  const [data, setData] = useState(reviews)
  const [spoilerData, setSpoilerData] = useState<IMangaReview[] | null>(null)
  ...
  const handleClick = async () => {
    ...
    if (!buttonHasBeenClicked) {
      const news: IMangaReviews = await (
        await fetch(
          `https://api.jikan.moe/v4/manga/${params.id}/reviews?spoilers=true&preliminary=true`
        )
      ).json()
      const { data } = news
      console.log(data)
